...
that looks like. No scenes, and apart from two arrows only shapes and text.
...
| Rev bar, fuel and temperature bars | a bright bar inside a group with a `clip`, covered by a dark rect whose `x` is bound to the variable (`scale` and `offset` turn 50..130 C into 0..220 px) |
...
- A `rotation` property: a needle gauge is the obvious dashboard element
  and cannot be built. It is an affine transform, so far cheaper than
  perspective.
...
- Polygon (or path) shapes and a tint for images: the arrows would not
  need image files.
